-- Extract Snowflake Query History using Key-Pair Authentication
-- This version works with MFA-enabled accounts

-- Load the Snowflake extension
INSTALL snowflake FROM community;
LOAD snowflake;

-- Create Snowflake secret using key-pair authentication
-- Uses environment variables for credentials:
--   SNOWFLAKE_ACCOUNT
--   SNOWFLAKE_USER
--   SNOWFLAKE_PRIVATE_KEY_PATH (path to the .p8 file)
...
CREATE OR REPLACE SECRET snowflake_secret (
    ...
    AUTH_TYPE 'key_pair',
    PRIVATE_KEY getenv('SNOWFLAKE_PRIVATE_KEY_PATH'),
    DATABASE 'SNOWFLAKE',
