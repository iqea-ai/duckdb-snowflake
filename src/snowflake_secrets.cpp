...
void SnowflakeSecretsHelper::StoreCredentials(
    ClientContext &context, const std::string &profile_name,
...
SnowflakeSecretsHelper::GetCredentials(ClientContext &context,
  ...
  try {
    ...
    auto secret_entry =
        secret_manager.GetSecretByName(transaction, profile_name);
    if (!secret_entry) {
      throw InvalidInputException("Snowflake profile not found: " +
                                  profile_name);
    }

    // Cast to SnowflakeSecret to access the snowflake-specific fields
    auto *snowflake_secret =
        dynamic_cast<const SnowflakeSecret *>(secret_entry->secret.get());
    if (!snowflake_secret) {
      throw InvalidInputException("Invalid secret type for profile: " +
                                  profile_name);
    }
    ...
    std::cerr << "[DEBUG] GetCredentials: auth_type_str = '" << auth_type_str
              << "'" << std::endl;
    if (!auth_type_str.empty()) {
      ...
      if (auth_type_str == "oauth") {
        ...
        std::cerr << "[DEBUG] Set auth_type to OAUTH, token length = "
        ...
        config.private_key_passphrase =
            snowflake_secret->GetPrivateKeyPassphrase();
      } else if (auth_type_str == "ext_browser" ||
                 auth_type_str == "externalbrowser") {
      ...
      std::cerr << "[DEBUG] auth_type_str is empty, using default PASSWORD auth"
                << std::endl;
    }

  } catch (const std::exception &e) {
    throw InvalidInputException("Failed to retrieve credentials for profile '" +
                                profile_name + "': " + e.what());
  }
...
bool SnowflakeSecretsHelper::DeleteCredentials(
    ClientContext &context, const std::string &profile_name) {
  try {
    auto &secret_manager = SecretManager::Get(context);
    auto transaction = CatalogTransaction::GetSystemCatalogTransaction(context);
    ...
    secret_manager.DropSecretByName(transaction, profile_name,
    ...
    std::cerr << "Failed to delete credentials for profile '" << profile_name
              << "': " << e.what() << '\n';
    return false;
  }
}
...
SnowflakeSecretsHelper::ListProfiles(ClientContext &context) {
...
std::string
SnowflakeSecrets::StoreCredentials(const std::string &profile_name) {
...
std::string
SnowflakeSecrets::GetConnectionString(const std::string &profile_name) {
  // This is deprecated - users should use the new secrets manager approach
...
bool SnowflakeSecretsHelper::ValidateCredentials(
    ClientContext &context, const std::string &profile_name,
    int timeout_seconds) {
  try {
    ...
    try {
      ...
      AdbcStatusCode status =
          AdbcStatementNew(connection->GetConnection(), &statement, &error_obj);
      ...
      status =
          AdbcStatementExecuteQuery(&statement, &stream, nullptr, &error_obj);
      ...
      fprintf(stderr,
              "[Snowflake Validation] Connection test failed for profile\n");
      ...
      fprintf(
          stderr,
          "[Snowflake Validation] Unexpected error during connection test\n");
      fprintf(stderr, "  Error: %s\n", inner_e.what());
      return false;
    }
  } catch (const std::exception &e) {
    // Log the error but don't throw
    std::cerr << "Failed to validate credentials for profile '" << profile_name
              << "': " << e.what() << '\n';
    return false;
  }
}
...
bool SnowflakeSecretsHelper::ValidateCredentials(
    ClientContext &context, const std::string &username,
  ...
  try {
    ...
    // Log the error but don't throw
    fprintf(stderr,
            "[Snowflake Validation] Failed to validate credentials: %s\n",
            e.what());
    return false;
  }
}
