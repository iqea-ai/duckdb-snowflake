...
unique_ptr<ArrowArrayStreamWrapper> SnowflakeProduceArrowScan(uintptr_t factory_ptr,
	...
	if (factory->filter_pushdown_enabled || factory->projection_pushdown_enabled) {
		...
		// Log what DuckDB is passing us even when pushdown is disabled for debugging
		DPRINT("  DuckDB passed %llu projection columns (ignored since pushdown disabled)\n",
		       parameters.projected_columns.columns.size());
		if (parameters.filters) {
			DPRINT("  DuckDB passed %llu filters (ignored since pushdown disabled)\n",
			       parameters.filters->filters.size());
		}
		// Don't call UpdatePushdownParameters - keep factory->modified_query as the original query
	}
	...
	// Always set the SQL query before execution to ensure we use the modified query with pushdown
	// This handles the case where the statement was initialized during schema fetch but pushdown
	// parameters are only available now during execution
	{
		AdbcError set_error;
...
void SnowflakeArrowStreamFactory::UpdatePushdownParameters(const vector<string> &projection,
	...
	try {
		// Extract table name from base query (e.g., "SELECT * FROM database.schema.table")
		string table_name;
		...
		// Note: When filters are present, the filter column indices refer to positions in the
		// projection list (cols_to_project), NOT the original schema (column_names).
