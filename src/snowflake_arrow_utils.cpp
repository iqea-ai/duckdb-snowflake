...
SnowflakeProduceArrowScan(uintptr_t factory_ptr,
  ...
  DPRINT("SnowflakeProduceArrowScan: factory=%p, statement_initialized=%d\n",
         (void *)factory, factory->statement_initialized);
  DPRINT(
      "SnowflakeProduceArrowScan: pushdown enabled: filter=%d, projection=%d\n",
  ...
  if (factory->projection_pushdown_enabled) {
    ...
    for (size_t i = 0; i < parameters.projected_columns.columns.size(); i++) {
      DPRINT("  projected_column[%llu] = %s\n", i,
             parameters.projected_columns.columns[i].c_str());
    }
  }

  // Apply pushdown if enabled (this modifies the query before execution)
  if (factory->filter_pushdown_enabled ||
  ...
  if (!factory->statement_initialized) {
    ...
    AdbcStatusCode status = AdbcStatementNew(
        factory->connection->GetConnection(), &factory->statement, &error);
  ...
  // now during execution
  {
    ...
    AdbcStatusCode set_status = AdbcStatementSetSqlQuery(
        &factory->statement, factory->modified_query.c_str(), &set_error);
  ...
  AdbcStatusCode status = AdbcStatementExecuteQuery(
      &factory->statement, &adbc_stream, &rows_affected, &error);
  if (status != ADBC_STATUS_OK) {
...
void SnowflakeGetArrowSchema(ArrowArrayStream *factory_ptr,
                             ArrowSchema &schema) {
  auto factory = reinterpret_cast<SnowflakeArrowStreamFactory *>(factory_ptr);
  ...
  if (!factory->statement_initialized) {
    ...
    status = AdbcStatementSetSqlQuery(&factory->statement,
                                      factory->modified_query.c_str(), &error);
    if (status != ADBC_STATUS_OK) {
      std::string error_msg = "Failed to set query: ";
      if (error.message) {
        error_msg += error.message;
  ...
  AdbcStatusCode schema_status =
      AdbcStatementExecuteSchema(&factory->statement, &schema, &schema_error);
...
void SnowflakeArrowStreamFactory::UpdatePushdownParameters(
    const vector<string> &projection, TableFilterSet *filter_set) {
  ...
  try {
    ...
    if (from_pos != string::npos) {
      ...
      throw InvalidInputException(
          "Invalid base query format: missing FROM clause");
    }
    ...
    vector<string> filter_column_names =
        cols_to_project.empty() ? column_names : cols_to_project;
    modified_query = snowflake::SnowflakeQueryBuilder::BuildQuery(
        table_name, cols_to_project, filters_to_push, filter_column_names);
