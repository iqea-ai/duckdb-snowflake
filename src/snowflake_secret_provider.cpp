...
void SnowflakeSecret::Validate() const {
  ...
  if (!missing_fields.empty()) {
    throw InvalidInputException(
        "Snowflake secret is missing required fields: %s",
...
unique_ptr<BaseSecret> SnowflakeSecret::Deserialize(Deserializer &deserializer,
                                                    BaseSecret base_secret) {
  auto result = make_uniq<SnowflakeSecret>(
      base_secret.GetScope(), base_secret.GetProvider(), base_secret.GetName());
...
unique_ptr<BaseSecret> CreateSnowflakeSecret(ClientContext &context,
                                             CreateSecretInput &input) {
  // Create the secret with the provided scope and name
  auto secret =
      make_uniq<SnowflakeSecret>(input.scope, input.provider, input.name);
  ...
  vector<string> optional_fields = {
      "user",        "password", "warehouse", "schema",
  ...
  for (const auto &field : required_fields) {
    ...
    if (it == input.options.end()) {
      throw InvalidInputException("Snowflake secret requires field '%s'",
                                  field);
    }
...
void RegisterSnowflakeSecretType(DatabaseInstance &instance) {
  ...
  create_function.named_parameters["private_key_passphrase"] =
      LogicalType::VARCHAR;

  // Register the create function
  secret_manager.RegisterSecretFunction(create_function,
