...
//! SnowflakeQueryBuilder: AST-based query construction for filter and projection pushdown
//!
//! This class builds SQL queries by constructing a DuckDB AST programmatically from
//! DuckDB's pre-parsed filters and projections, then serializing to SQL.
//!
//! Architecture:
//!   User Query → DuckDB Parser → DuckDB Optimizer → Extracts filters/projections
//!                                                  ↓
//!   We receive: TableFilterSet (pre-parsed!) + vector<string> (column names)
//!                                                  ↓
//!   We build AST: SelectStatement → SelectNode → TableRef + Filters + Projections
//!                                                  ↓
