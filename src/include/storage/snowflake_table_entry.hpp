...
struct SnowflakeTableBindData : public FunctionData {
  ...
  bool Equals(const FunctionData &other) const override {
    ...
    return (database_name == other_data.database_name &&
            schema_name == other_data.schema_name &&
            table_name == other_data.table_name &&
            column_names == other_data.column_names &&
            column_types == other_data.column_types &&
...
public:
  SnowflakeTableEntry(Catalog &catalog, SchemaCatalogEntry &schema,
                      CreateTableInfo &info, shared_ptr<SnowflakeClient> client)
  ...
  const SnowflakeConfig &GetConfig() const { return client->GetConfig(); }
