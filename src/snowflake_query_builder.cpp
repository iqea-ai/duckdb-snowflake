...
string SnowflakeQueryBuilder::BuildQuery(
    const string &table_name, const vector<string> &projection_columns,
...
unique_ptr<ParsedExpression> SnowflakeQueryBuilder::BuildWhereExpression(
    TableFilterSet *filter_set, const vector<string> &column_names) {
  ...
  for (const auto &filter_pair : filter_set->filters) {
    ...
    if (column_idx >= column_names.size()) {
      throw InternalException(
          "Filter column index %llu out of range (have %llu columns)",
...
SnowflakeQueryBuilder::TransformFilter(const TableFilter &filter,
  ...
  case TableFilterType::CONSTANT_COMPARISON: {
    ...
    default:
      throw NotImplementedException(
          "Comparison type not supported for pushdown: %s",
    ...
    return make_uniq<ComparisonExpression>(
        comparison_type, std::move(column_ref), std::move(constant));
  }
  ...
  case TableFilterType::CONJUNCTION_AND: {
    ...
    if (conj_filter.child_filters.empty()) {
      throw InternalException(
          "CONJUNCTION_AND filter has no child filters for column '%s'",
          column_name.c_str());
    }
  ...
  case TableFilterType::OPTIONAL_FILTER: {
    ...
    if (!opt_filter.child_filter) {
      throw InternalException(
          "OPTIONAL_FILTER has no child filter for column '%s'",
          column_name.c_str());
    }
  ...
  case TableFilterType::IN_FILTER: {
    ...
    if (in_filter.values.empty()) {
      throw InternalException("IN_FILTER has no values for column '%s'",
                              column_name.c_str());
    }
    ...
    for (const auto &value : in_filter.values) {
      ...
      auto comparison = make_uniq<ComparisonExpression>(
          ExpressionType::COMPARE_EQUAL, std::move(col_ref),
          std::move(constant));
  ...
  case TableFilterType::CONJUNCTION_OR: {
    ...
    if (conj_filter.child_filters.empty()) {
      throw InternalException(
          "CONJUNCTION_OR filter has no child filters for column '%s'",
          column_name.c_str());
    }
  ...
  default:
    throw NotImplementedException(
        "Filter type %d on column '%s' is not supported for Snowflake "
        "pushdown. "
...
vector<unique_ptr<ParsedExpression>> SnowflakeQueryBuilder::BuildProjectionList(
