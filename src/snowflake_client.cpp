...
static std::string GetExtensionDirectory() {
  ...
  if (GetModuleHandleEx(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS |
                        ...
                        reinterpret_cast<LPCTSTR>(&GetExtensionDirectory),
                        &hModule)) {
    char path[MAX_PATH];
    if (GetModuleFileNameA(hModule, path, sizeof(path))) {
      ...
      std::string dir =
          (last_sep != std::string::npos) ? full_path.substr(0, last_sep) : ".";
  ...
  if (dladdr(reinterpret_cast<void *>(&GetExtensionDirectory), &info)) {
    ...
    std::string dir =
        (last_sep != std::string::npos) ? path.substr(0, last_sep) : ".";
...
SnowflakeClient::~SnowflakeClient() { Disconnect(); }
...
bool SnowflakeClient::IsConnected() const { return connected; }

const SnowflakeConfig &SnowflakeClient::GetConfig() const { return config; }
...
void SnowflakeClient::InitializeDatabase(const SnowflakeConfig &config) {
  ...
  // 3. Try system paths
#ifdef _WIN32
  search_paths.push_back(std::string("C:\\Windows\\System32\\") +
                         SNOWFLAKE_ADBC_LIB);
  search_paths.push_back(std::string("C:\\Program Files\\Snowflake\\") +
                         SNOWFLAKE_ADBC_LIB);
#else
  ...
  status =
      AdbcDatabaseSetOption(&database, "driver", driver_path.c_str(), &error);
  CheckError(status, "Failed to set Snowflake driver path", &error);

  // Set connection parameters
  status = AdbcDatabaseSetOption(&database, "adbc.snowflake.sql.account",
                                 config.account.c_str(), &error);
  ...
  case SnowflakeAuthType::PASSWORD:
    ...
    if (!config.username.empty()) {
      status = AdbcDatabaseSetOption(&database, "username",
                                     config.username.c_str(), &error);
      CheckError(status, "Failed to set username", &error);
    }
    if (!config.password.empty()) {
      status = AdbcDatabaseSetOption(&database, "password",
                                     config.password.c_str(), &error);
  ...
  case SnowflakeAuthType::OAUTH:
    ...
    if (!config.oauth_token.empty()) {
      std::cerr << "[DEBUG] Setting token (length: "
                << config.oauth_token.length() << ")" << std::endl;
      status = AdbcDatabaseSetOption(&database, "adbc.snowflake.sql.auth_token",
  ...
  case SnowflakeAuthType::KEY_PAIR:
    ...
    if (!config.private_key.empty()) {
      ...
      if (is_file_path && !config.private_key_passphrase.empty()) {
        ...
        if (!key_file.is_open()) {
          throw IOException("Failed to open private key file: " +
                            config.private_key);
        }
  ...
  case SnowflakeAuthType::OKTA:
    ...
    if (!config.okta_url.empty()) {
      status =
          AdbcDatabaseSetOption(&database, "adbc.snowflake.sql.auth_okta_url",
  ...
  if (!config.warehouse.empty()) {
    status = AdbcDatabaseSetOption(&database, "adbc.snowflake.sql.warehouse",
                                   config.warehouse.c_str(), &error);
    CheckError(status, "Failed to set warehouse", &error);
  }

  if (!config.database.empty()) {
    status = AdbcDatabaseSetOption(&database, "adbc.snowflake.sql.database",
                                   config.database.c_str(), &error);
    CheckError(status, "Failed to set database", &error);
  }

  if (!config.role.empty()) {
    status = AdbcDatabaseSetOption(&database, "adbc.snowflake.sql.role",
                                   config.role.c_str(), &error);
    CheckError(status, "Failed to set role", &error);
  }

  // Set query timeout
  std::string timeout_str = std::to_string(config.query_timeout);
  status = AdbcDatabaseSetOption(&database,
                                 "adbc.snowflake.sql.client_session_keep_alive",
...
void SnowflakeClient::CheckError(const AdbcStatusCode status,
  ...
  std::string error_message =
      operation + ": " +
      ((error && error->message) ? error->message : "Unknown ADBC error.");
  ...
  if (error && error->message) {
    ...
    if (msg.find("authentication") != std::string::npos ||
        msg.find("Authentication") != std::string::npos) {
      DPRINT("username and password\n");
    } else if (msg.find("account") != std::string::npos ||
               msg.find("Account") != std::string::npos) {
      DPRINT("account identifier format (e.g., 'account-name.region')\n");
    } else if (msg.find("warehouse") != std::string::npos ||
               msg.find("Warehouse") != std::string::npos) {
      DPRINT("warehouse name and permissions\n");
    } else if (msg.find("database") != std::string::npos ||
...
vector<string> SnowflakeClient::ListSchemas(ClientContext &context) {
  const string schema_query = "SELECT schema_name FROM " + config.database +
...
vector<string> SnowflakeClient::ListTables(ClientContext &context,
  ...
  const string table_name_query =
      "SELECT table_name FROM " + config.database +
...
vector<SnowflakeColumn>
SnowflakeClient::GetTableInfo(ClientContext &context, const string &schema,
                              const string &table_name) {
  const string upper_schema = StringUtil::Upper(schema);
  const string upper_table = StringUtil::Upper(table_name);

  const string table_info_query =
      "SELECT COLUMN_NAME, DATA_TYPE, IS_NULLABLE FROM " + config.database +
  ...
  const vector<string> expected_names = {"COLUMN_NAME", "DATA_TYPE",
                                         "IS_NULLABLE"};
  ...
  if (result.empty() || result[0].empty()) {
    throw CatalogException(
        "Cannot retrieve column information for table '%s.%s'. "
...
vector<vector<string>> SnowflakeClient::ExecuteAndGetStrings(
  ...
  CheckError(status, "Failed to set AdbcStatement with SQL query: " + query,
             &error);
  ...
  CheckError(status, "Failed to execute AdbcStatement with SQL query: " + query,
             &error);
  ...
  if (!expected_col_names.empty()) {
    if (static_cast<size_t>(schema.n_children) != expected_col_names.size()) {
      throw IOException("Expected " + to_string(expected_col_names.size()) +
                        " columns but got " + to_string(schema.n_children));
    }

    for (idx_t col_idx = 0; col_idx < expected_col_names.size(); col_idx++) {
      if (schema.children[col_idx]->name &&
          !StringUtil::CIEquals(schema.children[col_idx]->name,
                                expected_col_names[col_idx])) {
        throw IOException("Expected column '" + expected_col_names[col_idx] +
                          "' but got '" + schema.children[col_idx]->name + "'");
      }
    }
  }
  ...
  while (true) {
    ...
    if (return_code != 0) {
      throw IOException("ArrowArrayStream returned error code: " +
    ...
    for (idx_t col_idx = 0;
         col_idx < static_cast<idx_t>(arrow_array.n_children); col_idx++) {
      ArrowArray *column = arrow_array.children[col_idx];
      if (column && column->buffers &&
          static_cast<size_t>(column->n_buffers) >= 3) {
        // For string columns: buffer[0] is validity, buffer[1] is offsets,
        // buffer[2] is data
        const int32_t *offsets =
            static_cast<const int32_t *>(column->buffers[1]);
        ...
        for (int64_t row_idx = 0;
             row_idx < static_cast<int64_t>(column->length); row_idx++) {
  ...
  CheckError(AdbcStatementRelease(&statement, &error),
             "Failed to release AdbcStatement", &error);

  return results;
}

unique_ptr<DataChunk>
SnowflakeClient::ExecuteAndGetChunk(ClientContext &context, const string &query,
  ...
  if (!connected) {
    ...
    throw IOException(
        "Connection must be created before ExecuteAndGetChunk is called");
  }
  ...
  if (static_cast<size_t>(schema.n_children) != expected_types.size()) {
    throw IOException("Schema has " + std::to_string(schema.n_children) +
  ...
  ArrowTableFunction::PopulateArrowTableSchema(
      DBConfig::GetConfig(context), arrow_table, schema_wrapper.arrow_schema);
  actual_names = arrow_table.GetNames();
  actual_types = arrow_table.GetTypes();

  if (actual_types.size() != expected_types.size()) {
    throw IOException("Schema mismatch: expected " +
                      to_string(expected_types.size()) + " columns but got " +
                      to_string(actual_types.size()));
  }

  for (size_t name_idx = 0; name_idx < expected_names.size(); name_idx++) {
    if (!StringUtil::CIEquals(expected_names[name_idx],
                              actual_names[name_idx])) {
      throw IOException("Expected column '" + expected_names[name_idx] +
                        "' at position " + to_string(name_idx) + " but got '" +
                        actual_names[name_idx] + "'");
    }
  }
  ...
  while (true) {
    ...
    try {
      ArrowTableFunction::ArrowToDuckDB(local_state, arrow_table.GetColumns(),
                                        *temp_chunk, batch_count - 1);
