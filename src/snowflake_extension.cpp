...
inline void SnowflakeVersionScalarFun(DataChunk &args, ExpressionState &state,
                                      Vector &result) {
...
static void LoadInternal(ExtensionLoader &loader) {
  ...
  auto snowflake_version_function = ScalarFunction(
      "snowflake_version", {}, LogicalType::VARCHAR, SnowflakeVersionScalarFun);
  ...
  config.storage_extensions["snowflake"] =
      make_uniq<snowflake::SnowflakeStorageExtension>();
#else
  // ADBC not available - register a placeholder function that throws an error
  auto snowflake_scan_function = TableFunction(
      "snowflake_scan", {},
...
void SnowflakeExtension::Load(ExtensionLoader &loader) { LoadInternal(loader); }
...
extern "C" {
DUCKDB_CPP_EXTENSION_ENTRY(snowflake, loader) { duckdb::LoadInternal(loader); }
}
#endif
}
