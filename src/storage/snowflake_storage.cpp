...
SnowflakeAttach(optional_ptr<StorageExtensionInfo> storage_info,
  ...
  if (secret_entry != info.options.end()) {
    ...
    try {
      ...
      throw InvalidInputException(
          "Failed to retrieve Snowflake credentials from secret '%s': %s",
    ...
    throw InvalidInputException(
        "Snowflake ATTACH requires either a connection string or SECRET "
        "option. "
  ...
  if (options.access_mode != AccessMode::READ_ONLY) {
    throw NotImplementedException(
        "Snowflake currently only supports read-only access");
  }
  ...
  if (pushdown_entry != info.options.end()) {
    ...
    if (pushdown_value == "true" || pushdown_value == "TRUE" ||
        pushdown_value == "1") {
      snowflake_options.enable_pushdown = true;
      DPRINT("Pushdown ENABLED by user option\n");
    } else if (pushdown_value == "false" || pushdown_value == "FALSE" ||
               pushdown_value == "0") {
    ...
    DPRINT(
        "Pushdown DISABLED by default (no enable_pushdown option provided)\n");
  }
