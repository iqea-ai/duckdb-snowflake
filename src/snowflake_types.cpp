...
LogicalType SnowflakeTypeToLogicalType(const std::string &snowflake_type_str) {
  ...
  if (base_type == "DOUBLE" || base_type == "FLOAT8" ||
      base_type == "DOUBLEPRECISION") {
  ...
  if (base_type == "DECIMAL" || base_type == "NUMERIC") {
    ...
    if (close_paren_pos == std::string::npos) {
      throw InvalidInputException("Expected closing ')' for DECIMAL type: " +
                                  snowflake_type_str);
    }

    auto params =
        normalized_type.substr(paren_pos + 1, close_paren_pos - paren_pos - 1);
    ...
    if (comma_pos == std::string::npos) {
      ...
      try {
        ...
        throw ConversionException("Invalid precision '%s' in type: %s", params,
                                  snowflake_type_str);
      }
    } else {
      ...
      try {
        ...
        throw ConversionException("Invalid scale '%s' in type: %s", scale_str,
                                  snowflake_type_str);
      }
    }

    if (precision < 1 || precision > 38) {
      throw ConversionException("DECIMAL precision %d out of range (1-38)",
                                precision);
    }
    if (scale < 0 || scale > precision) {
      throw ConversionException("DECIMAL scale %d invalid (must be 0-%d)",
                                scale, precision);
    }
  ...
  if (base_type == "NUMBER") {
    ...
    auto close_paren_pos = normalized_type.find(')');
    if (close_paren_pos == std::string::npos) {
      throw InvalidInputException("Expected closing ')' for NUMBER type: " +
                                  snowflake_type_str);
    }
    ...
    if (temp_precision < 1 || temp_precision > 38) {
      throw ConversionException("NUMBER precision %d out of range (1-38)",
                                temp_precision);
    }
    if (temp_scale < 0 || temp_scale > temp_precision) {
      throw ConversionException("NUMBER scale %d invalid (must be 0-%d)",
                                temp_scale, temp_precision);
    }
...
LogicalType ConvertNumber(uint8_t precision, uint8_t scale) {
  ...
  if (scale == 0) {
    ...
    if (precision <= 18) {
      return LogicalType::BIGINT; // -9,223,372,036,854,775,808 to
