...
### Get the Latest Extension Build (v1.5.3)

Install DuckDB 1.5.3 (or newer) and then install the Snowflake extension directly from the community repository:
...
# 3. Move to DuckDB extensions directory (DuckDB v1.5.3)
mkdir -p ~/.duckdb/extensions/v1.5.3/<PLATFORM>
mv adbc_driver_snowflake/libadbc_driver_snowflake.so ~/.duckdb/extensions/v1.5.3/<PLATFORM>/
...
# Install (DuckDB v1.5.3)
mkdir -p ~/.duckdb/extensions/v1.5.3/linux_amd64
mv adbc_driver_snowflake/libadbc_driver_snowflake.so ~/.duckdb/extensions/v1.5.3/linux_amd64/
...
# Place in DuckDB extensions directory (DuckDB v1.5.3)
mkdir C:\Users\%USERNAME%\.duckdb\extensions\v1.5.3\windows_amd64
move libadbc_driver_snowflake.so C:\Users\%USERNAME%\.duckdb\extensions\v1.5.3\windows_amd64\
```
...
> **Requires DuckDB 1.5.3 build of the Snowflake extension.** Earlier versions of the extension do not include the pushdown planner improvements referenced below.
